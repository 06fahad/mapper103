...
HDR = graphics.h fontcache.h graphics_state.h easygl_constants.h graphics_types.h Surface.h graphics_automark.h
SRC = graphics.cpp fontcache.cpp graphics_state.cpp example.cpp graphics_types.cpp Surface.cpp
EXE = example
...
ifeq ($(PLATFORM),X11)
	...
	FLAGS += $(INCLUDE_FLAGS) $(WARN_CFLAGS) $(shell pkg-config --cflags freetype2) # evaluates to the correct include flags for the freetype headers
endif

$(EXE): graphics.o fontcache.o graphics_state.o example.o graphics_types.o Surface.o
...
# Back up the source, makefile and Visual Studio project & solution. 
	...
	zip ${BACKUP_FILENAME} $(SRC) $(HDR) makefile easygl.sln easygl.vcxproj *.png
